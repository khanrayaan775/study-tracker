...
export default function StudyTracker() {
  ...
  const updateBooks = (updatedBooks) => {
    ...
    try {
      window.storage.set('study-books', JSON.stringify(updatedBooks));
    } catch (e) {
  ...
  const updateSeries = (updatedSeries) => {
    ...
    try {
      window.storage.set('study-series', JSON.stringify(updatedSeries));
    } catch (e) {
      console.error('Save error:', e);
    }
  };
  ...
  const saveName = (name) => {
    ...
    try {
      window.storage.set('user-name', name.trim());
    } catch (e) {
      console.error('Save error:', e);
    }
  };
  ...
  const toggleTheme = () => {
    ...
    try {
      window.storage.set('user-theme', newTheme ? 'dark' : 'light');
    } catch (e) {
      console.error('Save error:', e);
    }
  };
  ...
  if (!userName) {
    return (
      <div style={{ padding: '2rem 1.5rem', maxWidth: '700px', margin: '0 auto', minHeight: '100vh', background: colors.bg, display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center' }}>
        <style>{`
          @keyframes fadeIn {
  ...
  if (view === 'dashboard') {
    ...
    return (
      <div style={{ padding: '2rem 1.5rem', maxWidth: '700px', margin: '0 auto', minHeight: '100vh', background: colors.bg }}>
  ...
  // DAILY VIEW
  return (
    <div style={{ padding: '2rem 1.5rem', maxWidth: '650px', margin: '0 auto', minHeight: '100vh', background: colors.bg }}>
      <style>{`
        :root {
          --bg-primary: ${isDarkMode ? '#1a1a1a' : '#f5f5f5'};
          --bg-secondary: ${isDarkMode ? '#2a2a2a' : '#ffffff'};
      ...
      {books.length === 0 && series.length === 0 ? (
        <div style={{ padding: '3rem 1.5rem', textAlign: 'center', color: colors.textSecondary, background: colors.bgPrimary, borderRadius: 'var(--border-radius-lg)', border: `0.5px solid ${colors.border}` }}>
          <p style={{ fontSize: '16px', margin: 0 }}>No items yet.</p>
        </div>
      ) : (
          ...
          {books.map((book, idx) => {
            ...
            return (
              ...
              >
                ...
                {book.author && <p style={{ margin: '0 0 1rem 0', fontSize: '14px', color: colors.textSecondary }}>by {book.author}</p>}
          ...
          {series.map(s => {
            ...
            return (
              ...
              >
                ...
                {s.instructor && <p style={{ margin: '0 0 1rem 0', fontSize: '14px', color: colors.textSecondary }}>by {s.instructor}</p>}
                ...
                <div style={{ padding: '1.5rem', background: colors.bgSecondary, borderRadius: 'var(--border-radius-md)', marginBottom: '1rem' }}>
                  <div style={{ marginBottom: '1.5rem' }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '0.75rem' }}>
                      <button onClick={() => updateSeries(series.map(x => x.id === s.id ? { ...x, lecturesCompleted: Math.min(x.totalLectures, x.lecturesCompleted + task.itemsPerDay) } : x))} style={{ padding: '0.75rem', fontSize: '13px', border: '0.5px solid var(--color-border-secondary)', background: 'var(--color-background-primary)', borderRadius: 'var(--border-radius-md)', cursor: 'pointer', fontWeight: '500' }}>
                        Today's quota (+{task.itemsPerDay})
                      </button>
